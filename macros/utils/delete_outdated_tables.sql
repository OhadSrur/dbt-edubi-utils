{# Source: https://discourse.getdbt.com/t/faq-cleaning-up-removed-models-from-your-production-schema/113/3 #}
{# Command: dbt run-operation delete_outdated_tables --args "{schema: [foo, bar, baz]}" #}
{% macro delete_outdated_tables(schema,commit=FALSE) %} 
  ...
  {%- for to_delete in load_result('get_outdated_tables')['data'] %} 
    {% call statement(auto_begin=True) -%}
      ...
      {# {% do log('drop ' ~ to_delete[2] ~ ' if exists "' ~ to_delete[0] ~'"."' ~ to_delete[1] ~'" cascade;', info=True) %} #}
