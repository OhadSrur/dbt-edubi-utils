...
{% macro extract_year_level_from_code(string_col,position='left',default_none_int_value='null') %}
        ...
        when {{ position }}({{string_col}},2) ~ '^[0-9]+$'  and {{ position }}({{string_col}},2)::int between 0 and 13 then {{ position }}({{string_col}},2)::int
        when {{ position }}({{string_col}},2) ~ '^[0-9]+$'  and {{ position }}({{string_col}},2)::int > 13 then null
